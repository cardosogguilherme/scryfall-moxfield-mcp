...
**Cloudflare is stricter on the datacenter egress than on a home IP — the fingerprint must be current.** Resolved 2026-09-01. Symptom: all three Moxfield tools 403'd from Render while the identical code returned 200 from a residential Windows IP under `curl_cffi` 0.9.0 *and* 0.13.0, with or without our headers — so neither the version pin nor the header set was the whole story. What fixed it in the cloud was `impersonate="chrome"` on `curl_cffi` 0.13.0 (chrome136) *plus* removing the hand-written `Chrome/120.0` User-Agent that contradicted the TLS fingerprint. Read that as: from a datacenter ASN, Cloudflare scores hard enough that a stale profile or a UA/fingerprint mismatch tips it over, while the same request sails through from residential. **So when Moxfield 403s in the cloud again, bump the `curl_cffi` pin first** — do not add a UA, and do not reach for credentials (a public deck id 403s identically to an owned one, so creds are never implicated). `_MoxfieldHTTPError` captures `cf-ray` / `cf-mitigated` / `server` and a 200-char body snippet, so the next failure states whether it is a Cloudflare edge block or a Moxfield application error. Verify a change from the deploy, not from your laptop: `tests/moxfield/test_live_cloudflare.py` passes locally either way.
